...
## Current state
- versionName "1.99.2"; versionCode 114 (plain int, bumps every CI build). VERSION NUMBERS ONLY GO FORWARD — the rolling GitHub release keys the updater on version numbers, so going backward breaks it (see below). Pre-2.0 dev builds march as 1.99.x.
- v1.99.2 QoL (folds into 2.0.0 changelog): (1) RESET ALL two-tap confirm — first tap arms "TAP AGAIN TO WIPE", auto-disarms 3s; (2) OPEN TAB gained "last tab" option (cfg_start=2 → reopen on cfg_lasttab); tab clicks persist cfg_lasttab.
