...
## Current state
- versionName "1.99.3"; versionCode 115 (plain int, bumps every CI build). VERSION NUMBERS ONLY GO FORWARD — the rolling GitHub release keys the updater on version numbers, so going backward breaks it (see below). Pre-2.0 dev builds march as 1.99.x.
- QoL so far (folds into 2.0.0 changelog): v1.99.2 — RESET ALL two-tap confirm ("TAP AGAIN TO WIPE", auto-disarms 3s); OPEN TAB "last tab" option (cfg_start=2 → reopen on cfg_lasttab, persisted on tab tap). v1.99.3 — pool-grid one-step UNDO bar (↶ reverses last copies-seen mark; in-place via refreshUndoBar(), cleared on reset/close; recordSeenUndo only arms when count actually moved); GOLD tab quick-set presets (0/10/20/30/50) + tap the streak number to flip W↔L.
