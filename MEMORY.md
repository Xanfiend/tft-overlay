...
## Current state
- versionName "1.99.4"; versionCode 116 (plain int, bumps every CI build). VERSION NUMBERS ONLY GO FORWARD — the rolling GitHub release keys the updater on version numbers, so going backward breaks it (see below). Pre-2.0 dev builds march as 1.99.x.
- QoL so far (folds into 2.0.0 changelog): v1.99.2 — RESET ALL two-tap confirm; OPEN TAB "last tab" option. v1.99.3 — pool-grid UNDO bar; GOLD quick-set presets (0/10/20/30/50); streak number tap to flip W↔L. v1.99.4 — sigil auto-dim (idle 8s → fade to 0.18 alpha, any touch snaps back; scheduleDim/cancelDim helpers; cleaned up in onDestroy).
