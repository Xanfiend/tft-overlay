...
- **Phase 2 (enemy items) = TODO after P1.** Item icons aren't text → need visual icon-ID like SetIcons (bundle full-item PNGs, match popup item slots). Then OppScout weights threats by items.
- Data model already future-proofed: enemy unit = {name, stars, items[]}; oppboard storage "name|stars" extends to "name|stars|item1,item2,item3" (backward-compatible).
