...
- UI polish (06-22, versionCode 111, folds into 2.0): NOTE most plan-file beautification was ALREADY shipped in v1.11.x/v1.12.x (gradient box()/shape() with pressed StateListDrawable, active-tab gold underline, section-header rules, pulseGlow). Added the two missing bits: active-tab underline is now a center-bright LEFT_RIGHT gold gradient (was flat bar); addSecHdr caps each rule with a small dimmed ✦ ornament.
- DEV DIAGNOSTICS (06-22, versionCode 111): dev mode already existed (tap version label 7x → cfg_devmode → SCAN FROM IMAGE + DEBUG LOG ring buffer). Added a copyable "◇ DIAGNOSTICS" card in SETUP's dev block: devDiagnostics() dumps app ver/code, device, android API, screen WxH/density/orientation, accessibility-bound, root/emu, lvl/gold/streak/stage, calibration (landscapeGrid + oppPortraits n/7), set+builds patch. Pure reads.
...
- (4) NEXT (the headline): opponent-scan automation — LIVE-TEST GATED (needs laptop/emulator + real game). Two phases:
  - Phase 1 = champions + stars: calibrate the up-to-7 enemy-portrait tap positions (mirror the planner-cal flow, store in Pool cal_* keys); "SCAN ALL OPPONENTS" loops: tap portrait -> existing popup OCR scan -> file into slot 1-7 -> next. Timing constants + portrait positions tuned live.
