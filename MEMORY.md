...
- Changelog history: relabel existing entries into semver (features = x.y.0, bugfixes = x.y.z under nearest feature); KEEP entries (don't erase), but still collapse the ~15 near-identical probe-dot calibration fixes into 1-2 v1.9.x lines. Safe to do now — no users yet (only the dev).
